using UnityEngine;
using UnityEditor;
public class MultiPlayerBuild : MonoBehaviour
{
    #region Window
    ...
    static void PerformWin64Build(int playerCount)
    {
        ...
        for (int i = 1; i <= playerCount; i++)
        {
            BuildPipeline.BuildPlayer(
                GetScenePaths(), "Builds/Win64/" + GetProjectName() + i.ToString() + "/" + GetProjectName() + i.ToString() + ".exe",
    ...
    static void PerformMacBuild(int playerCount)
    {
        ...
        for (int i = 1; i <= playerCount; i++)
        {
            BuildPipeline.BuildPlayer(GetScenePaths(),
                "Builds/Mac/" + GetProjectName() + i.ToString() + "/" + GetProjectName() + i.ToString() + ".app",
